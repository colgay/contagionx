...
public plugin_init()
{
	...
	static cdata[Class_e], cdata2[Class_e], mdata[ClassMethod_e], vdata[ClassVar_e];
	new len, p;
	...
	for (new i = 0; i < g_ClassCount; i++)
	{
		...
		miter = TrieIterCreate(cdata[Class_Methods]);
		{
			while (!TrieIterEnded(miter))
			{
				...
				TrieIterGetArray(miter, mdata, ClassMethod_e);
...
public Trie:native_method(plugin_id, num_params)
{
	...
	else
	{
		...
		for (new i = 4; i <= num_params; i++)
		{
			...
			switch (type)
			{
				case FP_CELL, FP_FLOAT: 	mdata[Method_Params][len++] = FP_CELL;
				case FP_VAL_BYREF: 			mdata[Method_Params][len++] = FP_VAL_BYREF;
				case FP_ARRAY: 				mdata[Method_Params][len++] = FP_ARRAY;
				case FP_STRING: 			mdata[Method_Params][len++] = FP_STRING;
				default:
				{
	...
	TrieSetArray(cdata[Class_Methods], method_name, mdata, ClassMethod_e);
...
public Object:native_new(plugin_id, num_params)
{
	...
	static ctor[64], mdata[ClassMethod_e];
	get_string(2, ctor, charsmax(ctor));
	ArrayGetArray(g_aClasses, _:class_id, cdata);

	if (ctor[0])
	{
		if (!TrieGetArray(cdata[Class_Methods], ctor, mdata, ClassMethod_e))
		{
			log_error(AMX_ERR_NATIVE, "[OO] Constructor (%s) does not exist", ctor);
			return @null;
		}

		if (mdata[Method_Type] != MT_CTOR)
		{
...
public native_delete(plugin_id, num_params)
{
	...
	static cdata[Class_e], mdata[ClassMethod_e];
	...
	do {
		...
		if (cdata[Class_Dtor][0])
		{
			TrieGetArray(cdata[Class_Methods], cdata[Class_Dtor], mdata, ClassMethod_e);
...
public native_call(plugin_id, num_params)
{
	...
	if (strtok2(method_name, class, charsmax(class), name, charsmax(name), '@') != -1)
	{
		...
		if (!TrieGetArray(cdata[Class_Methods], name, mdata, ClassMethod_e))
		{
	...
	do {
		...
		if (TrieGetArray(cdata[Class_Methods], method_name, mdata, ClassMethod_e))
		{
			PushStackCell(g_ThisStack, odata[Object_Id]);
			callfunc_begin_i(mdata[Method_FuncId], mdata[Method_PluginId]);
			new ret = CallFuncEnd(3, mdata[Method_Params], num_params);
...
public any:native_get(plugin_id, num_params)
{
	...
	else if (vdata[Var_DataType] == DT_CELL)
	{
