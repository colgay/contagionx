...
{
	emit_sound(player, channel, array[random(size)], volume, attn, flags, pitch);
}
...
stock sendScreenShake(id, Float:amplitude, Float:frequency, Float:duration, bool:external=false)
{
	...
	if (external)
	{
		emessage_begin(MSG_ONE_UNRELIABLE, msgScreenShake, _, id);
		ewrite_short(fixedUnsigned16(amplitude, 1<<12));
		ewrite_short(fixedUnsigned16(duration, 1<<12));
		ewrite_short(fixedUnsigned16(frequency, 1<<8));
		emessage_end();
	}
	else
	{
