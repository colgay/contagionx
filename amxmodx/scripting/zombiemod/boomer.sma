const BOOMER_HP = 1000;
const BOOMER_HP2 = 500;
...
public Boomer::Init()
{
	...
	register_clcmd("testboom", "CmdTestBoom");
}
...
public CmdTestBoom(id)
{
	if (is_user_alive(id))
	{
		sendScreenFade(id, 5.0, 1.0, FFADE_IN, {0, 255, 0}, 150);
		sendScreenShake(id, 15.0, 7.5, 4.0);
		sendDamage(id, 0, 0, DMG_ACID, Float:{0.0, 0.0, 0.0});
		setPlayerDataF(id, "m_flVelocityModifier", 0.5);
...
stock boomerExplode(id)
{
	...
	{
		if (!is_user_alive(player) || entity_range(id, player) > BOOMER_EXPLODE_RADIUS || id == player)
			continue;
			
		if (!isZombie(player))
		{
			new Float:ratio = (1.0 - entity_range(id, player) / BOOMER_EXPLODE_RADIUS);
			new Float:damage =  floatmax(ratio * BOOMER_EXPLODE_DAMAGE_MAX, BOOMER_EXPLODE_DAMAGE_MIN)
			...
			/*
			sendScreenShake(player, (ratio * 15.0), (ratio * 7.5), 1.0 + (ratio * 4.0));
			sendScreenFade(player, (5.0 * ratio), (2.0 * ratio), FFADE_IN, {0, 255, 0}, 150);
